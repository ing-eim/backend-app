...
from app.excel_processor import process_excel, process_incidencias, process_pipeline_transporte, process_pipeline_comercial, process_disponibilidad_transporte, process_factoraje, process_relacion_pago, process_evidencias_pendientes, process_venta_perdida, process_pronostico_cobranza, process_presupuesto
import json
...
async def procesar_presupuesto(file: UploadFile = File(...), current_user: models.Usuario = Depends(get_current_user)):
    ...
    try:
        ...
        # Open DB session and call process_presupuesto inside single transaction
        try:
            from app.database import SessionLocal
            with SessionLocal() as db:
                try:
                    try:
                        ops_logger.info(f"Procesando presupuesto archivo={name_only} usuario={current_user.nombre_usuario}")
                    except Exception:
                        pass
                    inserted = process_presupuesto(file_location, db=db, username=current_user.nombre_usuario, original_name=name_only)
                    # commit once
                    db.commit()
                except Exception as pi_err:
                    tb = traceback.format_exc()
                    ops_logger.error(f"Error procesando presupuesto y ejecutando SP: {pi_err}\n{tb}")
                    try:
                        db.rollback()
                    except Exception:
                        pass
                    try:
                        ...
                        ops_logger.warning(f"No se pudo eliminar el archivo temporal {file_location}: {rm_err}")
                    return JSONResponse(status_code=500, content={"rows_inserted": 0, "error": "Error al procesar archivo presupuesto; verificar api.log"})
        except Exception:
            # Fallback: try to parse without DB (but we cannot insert), return error
            try:
                import pandas as _pd
            ...
            try:
                ...
                pass
            return JSONResponse(status_code=500, content={"rows_inserted": 0, "error": "No se pudo abrir sesión DB para insertar presupuesto; verificar api.log"})
        ...
        ops_logger.error(f"Error procesando archivo presupuesto {name_only}: {e}")
        raise HTTPException(status_code=400, detail=f"Error procesando archivo: {str(e)}")
